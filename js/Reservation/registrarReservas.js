...
function registrar() {
    ...
    let datos={
        ...
        Reservation:{"id":$("#Reservation").val()}
    }
    ...
    if (validar()){
        $.ajax({
            ...
            url: "http://localhost:8081/api/Reservationtion/save",
...
function armaListaReservations(items) {
    $("#listado").html("");
    $("#listado").show(500);
    //define variable javascript con la definicion inicial de la tabla, la primera fila y los
    //encabezados o títulos de la tabla
    var lista = ` <option value="">--selecciona una cabaña--</option>`;
    ...
    //accede al elemento con id 'listado' y adiciona la tabla de datos a su html
    $("#Reservation").html(lista);
}

function listarRerservas() {
    $.ajax({
        // la URL para la petición (url: "url al recurso o endpoint")
        ...
        success: function (respuesta) {
            ...
            //recibe el arreglo 'items' de la respuesta a la petición
            armaListaReservations(respuesta);
        },
